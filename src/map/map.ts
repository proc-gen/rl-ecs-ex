...
export class Map {
    ...
    getPath(start: Vector2, end: Vector2) {
        const astar = new AStar(end.x, end.y, this.passableCallBack.bind(this), { topology: 4 })
        const path: Vector2[] = []
    ...
    passableCallBack(x: number, y: number) {
        ...
        if (this.isWalkable(x, y)) {
            ...
            if (entities.length === 0 || entities.find(a => hasComponent(this.world, a, BlockerComponent)) === undefined) {
                return true
            } else if (entities.find(a => hasComponent(this.world, a, BlockerComponent) && hasComponent(this.world, a, PlayerComponent))) {
                return true
            }
        }

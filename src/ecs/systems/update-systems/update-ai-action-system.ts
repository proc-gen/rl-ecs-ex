...
import { ActionComponent, PlayerComponent, PositionComponent } from "../../components";
import { Map } from "../../../map";
...
export class UpdateAiActionSystem implements UpdateSystem {
    ...
    update(world: World, entity: EntityId) {
        if(hasComponent(world, entity, ActionComponent) && 
            !hasComponent(world, entity, PlayerComponent)){
            const aiPosition = PositionComponent.position[entity]
            if(this.playerFOV.find(a => a.x === aiPosition.x && a.y === aiPosition.y) !== undefined){

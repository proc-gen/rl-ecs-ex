...
- ~~More map generation styles~~
- ~~Game Over Stats + Other Minor Things~~
...
### Level Up Window Shenanigans

Two different issues here, the first of which is that in my haste to fix the multiple turns issue in `part-new-18`, I didn't kill enough enemies to check the level up window. This has been fixed in `part-new-19` and the game is no longer in a frozen state.

The other issue was that although base stats were updated appropriately, the calculated stats using the combination of base stats and equipment stats were not getting calculated. Quite annoying when you don't deal as much damage as you'd expect or take more damage than intended. This has also been fixed in `part-new-19`.
...
### Part New 19 - Ranged Enemies and Revamped AI

One of the overall smaller updates in terms of overall number of lines changed, including the two errata that were fixed, but is probably the biggest change in terms of gameplay since the original tutorial itself.

Firstly, enemies now have their own FOV and aren't dependent on being in the player's FOV to have a turn. To make them even more intelligent, they will remember the last place they saw the player and continue there even if the player is no longer visible.

Several new enemies have been introduced to make the early game a little more palatable, but also to showcase the ranged AI abilities. Goblin Slingers start with a sling to lob stones at the player, and Troll Archers similarly get a bow. Both are a little squishier than their Goblin or Troll counterparts, but provide more XP because you have to close the distance in order to kill them.

All of this could be extended to only allow the enemies to chase the player a certain distance away from their location, perhaps as a personality trait or something else. That, like many other things, is a topic for another day. Next up will be some major upgrades to the map generation.

### Part New 20 - More Map Generation

Up to this point, we've really only used one type of generator for our maps, which is kind of boring. ROT.js has several built-in map generation methods with plenty of customizability, and I chose three to implement here. Without diving into the details behind each one, they are:

- Maze Generator
  - Like the title says, it makes mazes
  - Dimensions were scaled up so that corridors are always 2 tiles wide instead of 1
- Cellular Generator
  - This one uses cellular automata to create a cave-like appearance
- Dungeon Generator
  - This one will look similar to the existing algorithm with rooms and hallways

For the maze and cellular generators, a different approach to placing entities, the start, and stairway positions was required. To keep it simple, there's a list of all traversible spaces kept from the ROT.js generator as floor tiles are applied to the map. Random positions are selected out of that array for everything, and there's also a requirement that the stairs be at least 25 spaces away from the start so that it's not immediately accessible.

While I would've preferred to include some other generator choices in addition to the ROT.js ones, binary space partitioning especially, it's getting very close to 7DRL 2026 and I still need to work on a game design document. The next section will likely be the last until 7DRL is over.

### Part New 21 - Game Over Screen and other nice things

It's a bit anticlimactic to lose and be sent back to the main menu without any other information. The main idea of this piece of code is to add a game over screen and log things that the player does.

A couple minor fixes like having the FOV functions actually respect the range provided as a distance. The base implementation provided by ROT.js is giving a square based on the provided range, so this just adds an additional distance check using the function already written for Vector2.

The other fix is that any save game that exists should get deleted when starting a new run or continuing an existing one. It's not really a roguelike if you can keep starting from an old save, right?

For quality of life, a help window now has instructions on how to play the game. There really are a lot of different actions that can be taken, and you don't want the player to get frustrated figuring out what keys do what. Not everyone has the benefit of reading your code to learn how things work!